...
const authRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/auth/login", async (req, reply) => {
    ...
    return reply.send({
      token,
      user: { id: user.id, email: user.email, role: user.role, active: user.active, name: user.name },
    });
  });

  // POST /auth/logout (client just deletes token, nothing server-side needed)
  ...
  app.get("/auth/me", { preHandler: (app as any).auth }, async (req: any, reply) => {
    ...
    return reply.send({
      user: { id: me.id, email: me.email, role: me.role, active: me.active, name: me.name },
    });
  });
